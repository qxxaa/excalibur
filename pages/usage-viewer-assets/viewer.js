document.addEventListener("DOMContentLoaded", () => {
  ...
  const rememberCredential = document.getElementById("remember-credential")
  ...
  const DEFAULT_TOKEN_USAGE_PERIOD = "today"
  ...
  const LEGACY_CREDENTIAL_PREFIX = "copilot-api.usage-viewer.credential."
  const CREDENTIAL_STORAGE_PREFIX = "copilot-api.usage-viewer.remembered."
  const sessionCredentials = new Map()
  let approvedEndpointOrigin = window.location.origin
  ...
  function normalizePeriod(value) {
    ...
    return LEGACY_PERIODS[value] || DEFAULT_TOKEN_USAGE_PERIOD
  }
  ...
  function readStoredApiKey(endpoint) {
    ...
    try {
      ...
      console.warn("Could not read stored credential:", getErrorMessage(error))
      return ""
    }
  }
  ...
  function writeStoredApiKey(endpoint, value) {
    ...
    try {
      ...
      console.warn("Could not store credential:", getErrorMessage(error))
    }
  }
  ...
  function removeLegacyStoredApiKeys() {
    try {
      const storage = window.localStorage
      // Old automatic persistence is not consent to remember a credential.
      const legacyKeys = []
      for (let index = 0; index < storage.length; index++) {
        const key = storage.key(index)
        if (
          key === LEGACY_API_KEY_STORAGE_KEY
          || key?.startsWith(LEGACY_CREDENTIAL_PREFIX)
        ) {
          legacyKeys.push(key)
        }
      }
      ...
      console.warn(
        "Could not remove legacy credentials:",
        getErrorMessage(error),
      )
    }
  }
  ...
  function loadStoredApiKey(endpoint) {
    return (
      sessionCredentials.get(getEndpointOrigin(endpoint))
      ?? readStoredApiKey(endpoint)
    )
  }

  function storeApiKey(value, endpoint) {
    const origin = getEndpointOrigin(endpoint)
    if (!origin || origin !== currentEndpointOrigin) return
    const trimmed = value.trim()
    sessionCredentials.set(origin, trimmed)
    if (trimmed && rememberCredential.checked) {
  ...
  function buildTokenUsageDailyUrl(usageEndpoint, period) {
    const url = new URL(deriveEndpointUrl(usageEndpoint, "token-usage/daily"))
    url.searchParams.set("period", period)
    return url.toString()
  }

  function buildTokenUsageEventsUrl(usageEndpoint, period, page) {
    const url = new URL(deriveEndpointUrl(usageEndpoint, "token-usage/events"))
    url.searchParams.set("period", period)
  ...
  async function fetchJson(url) {
    ...
    if (!origin || origin !== approvedEndpointOrigin) {
      throw new Error(
        "External endpoints require an explicit Refresh before requests are sent.",
      )
    }
    // Recheck at dispatch, including period changes and pagination.
    syncEndpointCredential()
    if (origin !== currentEndpointOrigin) {
      throw new Error("The selected endpoint changed. Refresh to load it.")
    }
  ...
  function updateControls() {
    ...
    fetchButtonLabel.textContent = state.isLoading ? "Refreshing..." : "Refresh"
  ...
  function resolveMetricValueFontSize(displayTexts) {
    const cards = Array.isArray(displayTexts) ? displayTexts : [displayTexts]
  ...
  function formatRangeText(range) {
    const start = formatRangeDate(range.start_ms)
    const end = formatRangeDate(range.end_ms - 1)
  ...
  function renderTokenUsageRangeText(period, range) {
    ...
    const label = labels[period] || labels.today

    if (!range || range.end_ms <= range.start_ms) {
      return label
    }

    return `${label}: ${formatRangeText(range)}`
  }
  ...
  function renderQuotaCard(title, details) {
    const quota = details && typeof details === "object" ? details : {}
    const numberOrNull = (value) =>
      typeof value === "number" && Number.isFinite(value) && value >= 0 ?
        value
      : null
    const entitlement = numberOrNull(quota.entitlement)
    ...
    const percentUsed =
      ...
      : Math.min(100, Math.max(0, 100 - percentage))
    const used =
      !unlimited && entitlement !== null && remaining !== null ?
        Math.max(0, entitlement - remaining).toLocaleString()
      : "N/A"
      ...
      <div class="metric-card">
        <div class="flex items-center justify-between gap-3">
          ...
          ${
            ...
            : `<span class="text-xs font-semibold" style="color: var(--color-gray);">${percentUsed === null ? "N/A" : `${percentUsed.toFixed(1)}% used`}</span>`
          }
        </div>
        <div class="mt-5">
          <div class="w-full progress-bar-bg h-2">
            <div class="progress-bar-fg h-2" style="width: ${unlimited ? 100 : (percentUsed ?? 0)}%; background-color: ${progressBarColor};"></div>
          </div>
        </div>
        <div class="mt-2 flex items-center justify-between gap-4 text-xs font-medium" style="color: var(--color-gray);">
          <span>${used} / ${
            unlimited ? "∞"
            : entitlement === null ? "N/A"
  ...
  function renderTokenUsageSection() {
    ...
    return `
      <section id="token-usage" class="dashboard-section">
        ...
        ${renderTokenUsageDailyTrend(dailySummary, state.tokenUsageDailyError)}
  ...
  function renderTokenUsageDailyTrend(dailySummary, error) {
    if (state.tokenUsagePeriod === "today") {
      return ""
    }

    const models = dailySummary?.byModel || []
    const selectedModel =
      models.some((model) => model.model === state.tokenUsageTrendModel) ?
  ...
  function renderTokenUsageTrendChart(dailySummary, selectedModel) {
    ...
    const visibleMetrics =
      state.tokenUsageTrendMetric === ALL_METRICS_VALUE ?
        metrics
      : metrics.filter((metric) => metric.key === state.tokenUsageTrendMetric)
    ...
    const lines = visibleMetrics
      .map((metric) => {
        const points = days
          .map((day, index) => {
            const totals = getDailyTrendTotals(day, selectedModel)
            return `${xForIndex(index)},${yForValue(totals[metric.key] || 0)}`
          })
          .join(" ")
        const circles = days
          .map((day, index) => {
            ...
            return `<circle cx="${xForIndex(index)}" cy="${yForValue(
              ...
              metric.color
            }" data-trend-day="${escapeHtml(day.date)}">
              <title>${escapeHtml(
    ...
    const hitRects = days
      .map((day, index) => {
        ...
        const title = [
          ...
          ...visibleMetrics.map(
            (metric) =>
              `${metric.label}: ${formatNumber(totals[metric.key] || 0)}`,
          ),
        ].join("\n")
        return `<rect x="${hitStart}" y="${padding.top}" width="${
          hitEnd - hitStart
        }" height="${plotHeight}" fill="transparent" style="cursor: pointer;" data-trend-day="${escapeHtml(day.date)}">
  ...
  function renderTokenUsageValueLines(value) {
    ...
    return lines
      .map(
        (line) =>
          `<span class="block whitespace-nowrap">${escapeHtml(line)}</span>`,
      )
      .join("")
  }
  ...
  async function fetchData(page = 1) {
    ...
    if (!endpoint) {
      ...
      state.tokenUsageDailyError = "Token usage requires a valid endpoint URL."
      state.tokenUsageSummaryError =
    ...
    try {
      ...
      } else if (isMissingTokenUsageEndpointError(dailyResult.reason)) {
        state.tokenUsageDailySummary = null
        state.tokenUsageDailyError = null
      } else {
        state.tokenUsageDailySummary = null
        state.tokenUsageDailyError = getErrorMessage(dailyResult.reason)
      }
  ...
  async function fetchTokenUsageEventsPage(page) {
    const endpoint = endpointUrlInput.value.trim()
    if (!endpoint) {
      state.tokenUsageEventsError = "Token usage requires a valid endpoint URL."
      render()
      return
    }
  ...
  function handleFormSubmit(event) {
    ...
    syncEndpointCredential()
    const origin = getEndpointOrigin(endpointUrlInput.value.trim())
    if (!origin) {
      state.error =
        "An HTTP or HTTPS endpoint without embedded credentials is required."
      render()
      return
    }
    approvedEndpointOrigin = origin
  ...
  function handleContentAreaClick(event) {
    ...
    if (trendMetric) {
      state.tokenUsageTrendMetric =
        trendMetric.getAttribute("data-trend-metric") || ALL_METRICS_VALUE
      render()
      return
    }
  ...
  function handleContentAreaChange(event) {
    ...
    state.tokenUsageTrendModel = event.target.value || ALL_MODELS_VALUE
    render()
  }
  ...
  function syncEndpointCredential() {
    ...
    rememberCredential.checked = Boolean(readStoredApiKey(endpoint))
  ...
  function init() {
    ...
    rememberCredential.addEventListener("change", () => {
      syncEndpointCredential()
      storeApiKey(apiKeyInput.value, endpointUrlInput.value.trim())
    })
    ...
    removeLegacyStoredApiKeys()
    syncEndpointCredential()

    if (
      endpointFromUrl
      && getEndpointOrigin(initialEndpoint) === window.location.origin
    ) {
      void fetchData()
    } else {
      if (endpointFromUrl)
        state.error =
          "Review this external endpoint, then select Refresh to connect."
      render()
    }
  }
